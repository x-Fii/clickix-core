...
import { Plus, Search, ShoppingCart, Eye, Trash2 } from 'lucide-react';
            ...
            <tbody>
              {filtered.map(pr => (
                <tr key={pr.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">{pr.pr_date ? format(new Date(pr.pr_date), 'dd MMM yyyy') : '—'}</td>
                  <td className="px-4 py-3 text-right font-mono text-sm">{pr.grand_total != null ? pr.grand_total.toFixed(2) : '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2.5 py-0.5 text-[11px] font-mono border rounded-full ${statusColors[pr.status] || statusColors.draft}`}>
